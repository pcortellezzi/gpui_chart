...
impl AxisRenderer {
    pub fn render_y_axis(
        ...
        let ticks = d3rs::scale::LinearScale::new()
            .domain(min, max)
            .range(1.0, 0.0)
            .ticks(10);
    ...
    pub fn render_x_axis(
        ...
        let axis_div = match edge {
            AxisEdge::Top => axis_div.top(px(0.0)).border_b_1(), 
            AxisEdge::Bottom => axis_div.bottom(px(0.0)).border_t_1(), 
            _ => axis_div,
        };

        axis_div.children(ticks.into_iter().map(|tick| {
