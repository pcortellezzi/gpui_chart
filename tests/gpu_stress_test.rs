...
use gpui_chart::gpu_renderer::{BladeComputeRenderer, GpuPoint, GpuAggregationMode};
...
fn test_lttb_ultimate_stress_100m() {
    ...
    let source_lttb = PolarsDataSource::new(df, "x", "y").with_aggregation_mode(AggregationMode::LTTB);
    
    let start_seq = Instant::now();
    let _ = source_lttb.iter_aggregated(0.0, n as f64, 2000).count();
    ...
    decimate_ilttb_arrays_par_into(&x_f64, &y_f64, 2000, &mut out_cpu);
    ...
    let gpu_points: Vec<GpuPoint> = (0..n).map(|i| GpuPoint { 
        x: i as f32, 
        y: (i as f32 * 0.001).sin() 
    }).collect();

    let start_gpu = Instant::now();
    let res_gpu = gpu_renderer.aggregate(&gpu_points, 0.0, n as f32, 2000, GpuAggregationMode::ILTTB).unwrap();
    let dur_gpu = start_gpu.elapsed();
    println!("ILTTB GPU (Blade)      took: {:?} (Transfers incl.)", dur_gpu);

    println!("\n--- FINAL VERDICT (100M Points) ---");
    println!("Parallel CPU vs Seq CPU: {:.2}x faster", dur_seq.as_secs_f64() / dur_par.as_secs_f64());
    println!("GPU vs Seq CPU:          {:.2}x faster", dur_seq.as_secs_f64() / dur_gpu.as_secs_f64());
    println!("GPU vs Parallel CPU:     {:.2}x faster", dur_par.as_secs_f64() / dur_gpu.as_secs_f64());
