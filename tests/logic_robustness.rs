#[cfg(test)]
mod tests {
    use gpui_chart::data_types::{AxisRange, PlotDataSource, VecDataSource};
    ...
    fn test_scale_zero_domain() {
        ...
        assert!(
            !mapped.is_nan(),
            "Mapped value should not be NaN for zero domain"
        );
        assert!(
            !mapped.is_infinite(),
            "Mapped value should not be Inf for zero domain"
        );

        // Ideally, if min==max, it should center the point?
        // Let's assert it produces a valid coordinate within range
        assert!(
            mapped >= 0.0 && mapped <= 100.0,
            "Mapped value {} should be within range [0, 100]",
            mapped
        );
    }
    ...
    fn test_axis_range_robustness() {
        ...
        let ticks = axis.ticks(10, None);

        // Ticks generation should not crash on flat range
        assert!(
            !ticks.is_empty(),
            "Should generate at least one tick even for flat range (or handle it)"
        );
