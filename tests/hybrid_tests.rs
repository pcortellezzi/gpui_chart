...
use gpui_chart::data_types::{PlotDataSource, PlotPoint, PlotData, ColorOp};
#[cfg(feature = "polars")]
use polars::prelude::*;
...
fn test_hybrid_data_integrity() {
    ...
    if let PlotData::Point(p) = &all_points[0] { assert_eq!(p.x, 0.0); }
    if let PlotData::Point(p) = &all_points[1499] { assert_eq!(p.x, 1499.0); }
}
...
fn test_hybrid_empty_historical() {
    ...
    hybrid.add_realtime(PlotData::Point(PlotPoint { x: 10.0, y: 20.0, color_op: ColorOp::None }));
...
fn test_hybrid_aggregation_consistency() {
    ...
    // Demander une agrégation sur une plage vide
    let mut output = Vec::new();
    hybrid.get_aggregated_data(20000.0, 30000.0, 100, &mut output);
    assert_eq!(output.len(), 0);

    // Demander une agrégation sur la plage existante
    hybrid.get_aggregated_data(0.0, 10000.0, 100, &mut output);
