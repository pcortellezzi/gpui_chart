...
fn test_aggregation_edge_cases_generic() {
    ...
    assert_eq!(decimate_min_max_slice(&data, 100, None, None).len(), 0);
    assert_eq!(decimate_m4_slice(&data, 100, None, None).len(), 0);
    assert_eq!(decimate_lttb_slice(&data, 100, None, None).len(), 0);
    ...
    assert_eq!(decimate_min_max_slice(&data, 100, None, None).len(), 1);
    assert_eq!(decimate_m4_slice(&data, 100, None, None).len(), 1);
    assert_eq!(decimate_lttb_slice(&data, 100, None, None).len(), 1);
    ...
    assert_eq!(decimate_min_max_slice(&data, 100, None, None).len(), 2);
    assert_eq!(decimate_m4_slice(&data, 100, None, None).len(), 2);
    assert_eq!(decimate_lttb_slice(&data, 100, None, None).len(), 2);

    // Max points < 1
    assert_eq!(decimate_min_max_slice(&data, 0, None, None).len(), 0);
    ...
    let data_many: Vec<PlotData> = (0..100)
        ...
        .collect();
    assert!(decimate_m4_slice(&data_many, 2, None, None).len() <= 2);
}
...
fn test_aggregation_nan_inf_generic() {
    ...
    // Min/Max
    let decimated = decimate_min_max_slice(&data, 2, None, None);
    assert_eq!(decimated.len(), 2);
    // Should not crash

    // M4
    let decimated = decimate_m4_slice(&data, 4, None, None);
    assert!(decimated.len() > 0);

    // LTTB
    let decimated = decimate_lttb_slice(&data, 3, None, None);
    assert!(decimated.len() > 0);
}
...
fn test_lttb_visual_stability() {
    ...
    let data: Vec<PlotData> = (0..100)
        ...
        .collect();
    let decimated = decimate_lttb_slice(&data, 10, None, None);
...
fn test_ilttb_parallel_correctness() {
    ...
    decimate_lttb_arrays_into(&x, &y, 100, &mut res_seq, None, None);

    // 2. Target: Parallel ILTTB
    let mut res_par = Vec::new();
    decimate_ilttb_arrays_par_into(&x, &y, 100, &mut res_par, None, None);
