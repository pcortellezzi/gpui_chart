use gpui_chart::aggregation::decimate_m4_arrays_par;
...
fn test_extreme_zoom_precision() {
    ...
    let res_tiny = decimate_m4_arrays_par(&x_tiny, &y, 100);
    assert!(res_tiny.len() > 0);
    assert!(!res_tiny.is_empty());

    // Huge range
    let x_huge: Vec<f64> = (0..n).map(|i| i as f64 * 1e15).collect();
    let res_huge = decimate_m4_arrays_par(&x_huge, &y, 100);
...
fn test_zero_range_stability() {
    ...
    let res = decimate_m4_arrays_par(&x, &y, 10);
